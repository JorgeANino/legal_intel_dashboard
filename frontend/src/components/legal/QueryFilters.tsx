...
export const QueryFilters = ({ onFilterChange, availableFilters, currentFilters }: Props) => {
  const [filters, setFilters] = useState<QueryFiltersType>(currentFilters || {});

  const handleFilterChange = (field: keyof QueryFiltersType, value: string[]) => {
    const newFilters = { ...filters, [field]: value.length > 0 ? value : undefined };
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {filters.jurisdictions?.map(jurisdiction => (
              <span key={jurisdiction} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                Jurisdiction: {jurisdiction}
              </span>
            ))}
            {filters.industries?.map(industry => (
              <span key={industry} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                Industry: {industry}
              </span>
            ))}
            {filters.geographies?.map(geography => (
              <span key={geography} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                Geography: {geography}
              </span>
            ))}
          </div>
