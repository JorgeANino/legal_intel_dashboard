...
const mockUseDocumentQuery = useDocumentQuery as jest.MockedFunction<typeof useDocumentQuery>;
...
describe('QueryPage', () => {
  ...
  it('renders query page with all elements', () => {
    ...
    expect(screen.getByText('Ask natural language questions about your legal documents')).toBeInTheDocument();
  });
  ...
  it('shows no results state when no query has been executed', () => {
    ...
    expect(screen.getByText('Enter a question above or try one of the example queries')).toBeInTheDocument();
  });
  ...
  it('does not show no results state when query results are available', () => {
    ...
    expect(screen.queryByText('Enter a question above or try one of the example queries')).not.toBeInTheDocument();
  });
  ...
  it('displays proper page title and description', () => {
    ...
    expect(screen.getByRole('heading', { level: 1, name: 'Query Documents' })).toBeInTheDocument();
    expect(screen.getByText('Ask natural language questions about your legal documents')).toBeInTheDocument();
  });

  it('handles query result state changes correctly', () => {
    // Test initial state (no results)
    const { rerender } = render(<QueryPage />, { wrapper: createTestWrapper() });
    expect(screen.getByText('Start Your Search')).toBeInTheDocument();
