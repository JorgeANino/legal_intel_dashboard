...
def create_franchise_agreement_pdf():
    ...
    Story.append(Paragraph('<b>4. TRADEMARKS AND PROPRIETARY MARKS</b>', styles['Heading2']))
    Story.append(Paragraph(
...
def create_license_agreement_docx():
    ...
    doc.add_paragraph('a) Install and use the Software on up to fifty (50) workstations', style='List Bullet')
    doc.add_paragraph(
        'b) Use the Software solely for Licensee\'s internal business operations in the oil and gas industry',
        style='List Bullet'
    )
    doc.add_paragraph('c) Access the Software\'s cloud-based features for data analysis and reporting', style='List Bullet')
    ...
    p.add_run('the parties have executed this Software License Agreement as of the date first written above.')
...
def create_lease_agreement_docx():
    ...
    p = doc.add_paragraph()
    p.add_run('IN WITNESS WHEREOF, ').bold = True
    p.add_run('the parties have executed this Commercial Lease Agreement as of the date first above written.')
...
if __name__ == '__main__':
    ...
    print("  4. 04_franchise_agreement_california.pdf - California Restaurant Franchise")
